...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case tribute:
        return tributeCardEffect(card, choice1, choice2, choice3, state, handPos, bonus, currentPlayer, nextPlayer);
...
int baronCardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus, int currentPlayer) {
    ...
    // User wants to discard an estate
    if (choice1 > 0) { 
        ...
        while(card_not_discarded) {
            ...
            if (state->hand[currentPlayer][p] == estate) { 
                ...
                discardCard(p, currentPlayer, state, 0); // discard card
...
void checkEstateSupply(int card, struct gameState *state) {
    int currentPlayer = whoseTurn(state);
    // check estate supply 
    if (supplyCount(card, state) > 0) {
        gainCard(card, state, 0, currentPlayer);//Gain an estate
    } 
    // check if supply runs out end game
    if (supplyCount(card, state) == 0) {
        isGameOver(state);
    }
}
...
// helper function to discard entire hand
void discardHand(int currentPlayer, struct gameState *state) {
...
int tributeCardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus, int currentPlayer, int nextPlayer) {
    int tributeRevealedCards[2] = {-1, -1};
    ...
    if ((state->discardCount[nextPlayer] + state->deckCount[nextPlayer]) <= 1) {
        ...
        // reveal top two cards and discard
        tributeRevealedCards[0] = state->deck[nextPlayer][state->deckCount[nextPlayer]-1];
